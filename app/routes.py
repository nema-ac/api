...
from . import wallet_data, logger, db
...
def link_wallet():
    ...
    if not all([solana_address, eth_address, signature_b64, message]):
        return jsonify({
            "success": False,
    ...
    if solana_address not in wallet_data:
        return jsonify({
            "success": False,
            "message": "Solana wallet is not eligible for airdrop"
        }), 400
    ...
    try:
        ...
        existing_link = WalletLink.query.get(solana_address)

        if existing_link:
            # Update the existing link
            existing_link.eth_address = eth_address
            message_response = "Wallet link updated successfully!"
        else:
            # Create a new link
            new_link = WalletLink(solana_address=solana_address, eth_address=eth_address)
            db.session.add(new_link)
            message_response = "Wallets linked successfully!"
...
@main.route('/check-link/<solana_address>')
def check_link(solana_address: str):
    try:
        ...
        if existing_link:
            return jsonify({
                "linked": True,
                "eth_address": existing_link.eth_address
            })
        else:
            return jsonify({
                "linked": False
